<script lang="ts">
	...
	const cardClicked = async (event: MouseEvent) => {
		if (typeof window !== 'undefined' && window.innerWidth > 640) {
			...
			showModal = true;
			try {
				readme = await fetchReadme(project.user || 'khulnasoft-lab', project.name);
			} catch (error) {
				console.error(error);
...
<Modal bind:showModal {project} {readme} />

<a
	class="project"
	href={`https://github.com/${project.user || config.githubUser}/${project.name}`}
	...
	<div class="right">
		...
		<div class="tags">
			...
			{#if project.stars && project.stars > 10}
				<p class="tag" title={`${project.stars} stars on GitHub`}>
...
<style lang="scss">
	.project {
		...
		&:hover {
			box-shadow:
				0 20px 25px -5px rgb(99 102 241 / 0.2),
				0 8px 10px -6px rgb(99 102 241 / 0.2);
			border-color: #1e293b;
